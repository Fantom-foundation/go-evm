...
machine; they are convenient to quickly test evm.
...
the evm HTTP service for example) may be opened from the Dockerfile (cf
...
attached to the default `~/.evm` directory, which is the default location
for `evml`.  
...
First, build the evm docker image (cf BUILD).
...
using the evm AMI (cf. BUILD), and connect them within this subnetwork.

Examples:

First, build the evm AMI (cf BUILD) and record its ID in the `ami`
